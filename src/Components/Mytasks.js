...
const Mytasks = ({tasks}) => {
    ...
    // console.log("hii" , tasks[0].date_added);
    ...
    // new Date().getDate() - task.date_added.slice(0,2)
    ...
    return ( 
        <div className="notes-container">
            <div className="sort-by-container">
                <motion.div className="filter-list" initial={{opacity:0}} animate={{opacity:1}} transition={{duration:1, delay:2}}>
                ...
                {filterClick ? (
                    <motion.div className="expand-content">
                        ...
                        <p onClick={() => !leastImportant ? setLeastImportant(true) && setMostImportant(false) : setLeastImportant(false) && setMostImportant(true)}>Deadline (high to low) {leastImportant ? <TiTick/> : null}</p>
                        <p onClick={() => !sortByDate ? setSortbyDate(true) : setSortbyDate(false) }>Date Added</p>
                    </motion.div>
            ...
            {tasks && <Tasklist tasks={tasks.filter((task) => task.status==='pending' && (mostImportant ? task.priority === 1  : task) && (leastImportant ? task.priority === 3 : task))}/>}
