...
const Createtask = (props) => {
    ...
    return ( 
        <div className="create-task-container">
            <motion.h3 style={{color : !darkMode ? "black" : "white"}} initial={{opacity:0}} animate={{opacity:1}} transition={{duration:1}} exit={{opacity:0}}>Enter a new task below</motion.h3>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-2" style={{borderColor : !darkMode ? "black" : "white", background : !darkMode ? "white" : "#004C74", color : !darkMode ? "black" : "white", boxShadow : !darkMode ? "6px 6px 2px #dba120" : "3px 3px 2px lightgrey"}}>
                        <div className="date-info">
                            ...
                            <input type="date" min={new Date().getFullYear() + "-" + (new Date().getMonth()+1 < 10 ? "0" : "") + (new Date().getMonth()+1) + "-" + new Date().getDate()} required value={deadline} onChange={(e) => setDeadline(e.target.value)}/>
                        </div>
